...
namespace cft
{
    ...
    // mov <control register> <- privledged instruction: will always cause exception
    // in usermode Why use this over int 3? Because anti-debugger routines will
    // typically pick up on that
    ...
    namespace helper
    {
        ...
        ZydisDisassembledInstruction get_ins(void* address)
        {
            ...
            if (ZYAN_SUCCESS(ZydisDisassembleIntel(ZYDIS_MACHINE_MODE_LONG_64, reinterpret_cast<std::uintptr_t>(address), address, ZYDIS_MAX_OPERAND_COUNT, &ins)))
                return ins;
        ...
        {
            // This map will act as a export name cache. We don't want to walk every
            // single module's exports every time we want to identify a function
            ...
            // Not going to comment this whole function, but we are manually fetching the
            // module list from the PEB and walking their export tables. Filling the cache
            // on the way in case a new module has appeared etc
        ...
        void* get_jmp_address(const ZydisDisassembledInstruction& instruction, const _EXCEPTION_POINTERS* exception_info)
        {
            ...
            switch (mnemonic)
            {
                ...
                case ZYDIS_MNEMONIC_JL:
                    taken = (bool)(flags & 0x80) != (bool)(flags & 0x800);
                    break;
                case ZYDIS_MNEMONIC_JNL:
                    taken = (bool)(flags & 0x80) == (bool)(flags & 0x800);
                    break;
                case ZYDIS_MNEMONIC_JLE:
                    taken = (flags & 0x40) || ((bool)(flags & 0x80) != (bool)(flags & 0x800));
                    break;
                ...
                case ZYDIS_MNEMONIC_LOOPE:
                    taken = (--ctx->Rcx != 0) && (flags & 0x40);
                    break;
                case ZYDIS_MNEMONIC_LOOPNE:
                    taken = (--ctx->Rcx != 0) && !(flags & 0x40);
                    break;
                case ZYDIS_MNEMONIC_JCXZ:
                    taken = (ctx->Rcx & 0xFFFF) == 0;
                    break;
                case ZYDIS_MNEMONIC_JECXZ:
                    taken = (ctx->Rcx & 0xFFFFFFFF) == 0;
                    break;
        ...
        void* get_next_address(const ZydisDisassembledInstruction& instruction, const _EXCEPTION_POINTERS* exception_info, std::string out_string)
        {
            ...
            if (instruction.info.mnemonic == ZYDIS_MNEMONIC_CALL)
            {
                ...
                // If its a call instruction, we want to check if there is a name we can put
        ...
        void safe_copy(void* address, const std::array<std::uint8_t, size>& bytes)
        {
            ...
            // We have to ensure that the page is writable, the chances of it being
            // writable by default is close to zero since we are overwriting code.
            VirtualProtect(address, size, PAGE_EXECUTE_READWRITE, &old_protect);

            std::memcpy(reinterpret_cast<void*>(address), bytes.data(), size);

            // We don't want to leave it RWX because that would probably fail any base
            // level anti-tamper checks.
    ...
    std::int32_t bp_handler(EXCEPTION_POINTERS* exception_info)
    {
        ...
        // Another benefit to using mov <control register> is that a privledged
        // instruction exception is very rare. If you make your exception something
        // less unique, lets say a access violation, you will have to rely on the fact
        // that the faulting address is not in the orig_ins_map and will be skipped
        // regardless.
        ...
        // If the iterator is at the end, it means we haven't found our faulting
        // address in the orig_ins_map. This either means some sort of bug has
        // occured, or more likely that the exception was not ours. In the case of mov
        // <control register>, we can imagine this would be quite rare but still very
        // important.
    ...
    {
        // VEH takes in a different struct as Wow64PrepareForException. Here is a
        // simple way to make them compatible so I only need one "bp_handler"
        // function.
    ...
    // Use EEH to handle breakpoints
    void init(void* start)
    {
        ZydisDecoderInit(&decoder, ZYDIS_MACHINE_MODE_LONG_64, ZYDIS_STACK_WIDTH_64);
        ZydisFormatterInit(&formatter, ZYDIS_FORMATTER_STYLE_INTEL);

        AddEarlyExceptionHandler(true, reinterpret_cast<PVECTORED_EXCEPTION_HANDLER>(&bp_handler));
    ...
    void cleanup()
    {
        ...
        RemoveEarlyExceptionHandler((PVECTORED_EXCEPTION_HANDLER)bp_handler);
    }
} // namespace cft
